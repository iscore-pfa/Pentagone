...
{
Presenter::Presenter(
    const Polygon::ProcessModel& layer,
    View* view,
    ...
    : LayerPresenter{ctx, parent}, m_layer{layer}, m_view{view}
{
  ...
  connect(&m_layer, &ProcessModel::splineChanged, this, [&] {
    m_view->setPolygon(m_layer.spline());
  });

  m_view->setPolygon(m_layer.spline());
  connect(m_view, &View::changed, this, [&] {
    CommandDispatcher<>{context().context.commandStack}.submit<ChangePolygon>(
        layer, m_view->spline());
  });
...
const Polygon::ProcessModel& Presenter::model() const
{
  return m_layer;
}
