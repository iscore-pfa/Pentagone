...
{
Presenter::Presenter(
    const Pentagone::ProcessModel& layer,
    View* view,
    ...
    : LayerPresenter{ctx, parent}, m_layer{layer}, m_view{view}
{
  ...
  connect(&m_layer, &ProcessModel::splineChanged, this, [&] {
    m_view->setPentagone(m_layer.spline());
  });

  m_view->setPentagone(m_layer.spline());
  connect(m_view, &View::changed, this, [&] {
    CommandDispatcher<>{context().context.commandStack}.submit<ChangePentagone>(
        layer, m_view->spline());
  });
...
const Pentagone::ProcessModel& Presenter::model() const
{
  return m_layer;
}
